...
def relative_position_encoding(depth, max_length=512, max_relative_position=127):
    ...
    # shape(max_len, max_len) 用于表示两个token的相对距离
    ...
    # 以下代码就是相对位置可以表示的大小
    # embeddings_tabel shape(vocab_size, depth) depth一般表示hidden_size 矩阵表示的是相对距离为1的embdding,相对距离为n的embedding
    embeddings_table = torch.zeros(vocab_size, depth)
    position = torch.arange(0, vocab_size, dtype=torch.float).unsqueeze(1)
    div_term = torch.exp(torch.arange(0, depth, 2).float() * (-math.log(10000.0) / depth))
    embeddings_table[:, 0::2] = torch.sin(position * div_term)
    embeddings_table[:, 1::2] = torch.cos(position * div_term)
    # embeddings_table shape(vocab_size, depth) -> (1, vocab_size, depth) -> (vocab_size, 1, depth) -> (vocab_size, depth)
    embeddings_table = embeddings_table.unsqueeze(0).transpose(0, 1).squeeze(1)

    # final_mat shape(max_len, max_len) final_mat表示当前max_len长度，两两位置的相对距离
    # flat_relative_positions_matrix shape(max_len * max_len, )
    flat_relative_positions_matrix = final_mat.view(-1)
    # shape(max_len*max_len, vocab_size) 每一个token需要和max_len个token有相对距离，所以一个token有max_len个向量表示 所以会有(max_len*max_len, vocab_size)的矩阵
    one_hot_relative_positions_matrix = torch.nn.functional.one_hot(flat_relative_positions_matrix,
                                                                    num_classes=vocab_size).float()
    # shape(max_len*max_len, depth)  embeddings_table shape(vocab_size, depth)
    ...
    # shape(max_len, max_len, depth) 含义就是第i个token与第j个token的相对距离，维度为depth，这个矩阵要体现到self-attention矩阵中。这是K矩阵
...
class NeZhaSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # shape(seq_len, seq_len, hidden_size)
        relations_keys = self.relative_positions_encoding[:to_seq_length, :to_seq_length, :].to(hidden_states.device)
        # query_layer shape(bs, num_heads, seq_len, head_size) -> (seq_len, bs, num_heads, head_size)
        query_layer_t = query_layer.permute(2, 0, 1, 3)

        # (seq_len, bs*num_heads, head_size)
        query_layer_r = query_layer_t.contiguous().view(from_seq_length, batch_size * num_attention_heads,
                                                        self.attention_head_size)
        # query_layer_r shape(seq_len, bs*num_heads, head_size)  relation_keys.permute(0,2,1) shape(seq_len, hidden_size, seq_len)
        # key_position_scores shape(seq_len, bs*num_heads, seq_len) 这样在self-attention矩阵中就有了相对距离信息了
        key_position_scores = torch.matmul(query_layer_r, relations_keys.permute(0, 2, 1))
        # shape(seq_len, bs, num_heads, seq_len)
