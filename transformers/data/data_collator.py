...
class DataCollatorForNgramMask(DataCollatorForLanguageModeling):
    ...
    def __call__(
        ...
        for e in examples:
            ...
            mask_labels.append(self._ngram_mask(ref_tokens))
        batch_mask = _collate_batch(mask_labels, self.tokenizer)
        inputs, labels = self.mask_tokens(batch_input, batch_mask)
        return {"input_ids": inputs, "labels": labels}

    def _ngram_mask(self, input_tokens: List[str], max_ngram=3, max_predictions=4):
        """
        Get 0/1 labels for masked tokens with whole word mask proxy
        """
        ...
        for index_set in cand_indexes:
            ...
            # If adding a whole-word mask would exceed the maximum number of
            # predictions, then just skip this candidate.
            if index_set in covered_indexes:
                continue
            if index_set < len(cand_indexes) - (n-1):
                for i in range(n):
                    ind = index_set + i
                    if ind in covered_indexes:
                        continue
                    covered_indexes.add(ind)
                    masked_lms.append(ind)
