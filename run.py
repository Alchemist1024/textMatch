...
from DataLoader import TextProcessor, convert_examples_to_features, convert_single_example
from sklearn.metrics import roc_curve, auc


class Predict:
    def __init__(self, config, model_path, label_path, bert_path='chinese-bert-wwm', max_seq_length=64):
        self.config = config
...
if __name__ == '__main__': 
    ...
    start = time.time()
    config = Config('data')
    model_path = 'data/match/save_dict/bert_v3_5.pth'
    label_path = 'data/match/label.txt'
    bert_path = 'pre_model'
    pre = Predict(config, model_path, label_path, bert_path)

    data_path = 'data/match/data_2_28/testA.tsv'
    to_path = 'data/match/data_2_28/res.tsv'
